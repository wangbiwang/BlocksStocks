...
const Questions_stock_supplementary = [
    '行业概念主板创业非ST；前1交易日涨跌幅正大单净额正资金流向流通市值且(M5和M10和M30和M60)均小于收盘价且(vol5和vol10和vol30和vol60)均小于vol1且(阳线或者涨停)；09:35股价>=前40交易日区间最高价不复权；09:35涨跌幅正；',
]
//指数板块相关
const Blocks = reactive({
    ...
    CheckedOptimumFN: () => {
        ...
        if (Blocks.CheckedOptimum._0935Red_10) {
            //红框前10
            Blocks.Data = Blocks.Data.map((el) => {
                // el.default = el.default.filter((ele) => (ele['序号'] <= 10)||(ele['今日红框']))
                el.default = el.default.slice(0, 2)
                return el
            })
        }
...
async function submitBlocks() {
    ...
    if (Dates.HistoryBtn == '历史') {
        stock_supplementary = `行业概念主板创业非ST；${us}涨跌幅正大单净额正资金流向且(M5和M10和M30和M60)均小于收盘价且(vol5和vol10和vol30和vol60)均小于vol1且(阳线或者涨停)；${us} 09:35股价>=${us}前40交易日区间最高价不复权；${us} 09:35涨跌幅正`
    }
...
async function handleBlocksData(res) {
    ...
    function handleArr(arr1, arr2, arr3, arr4) {
        ...
        return resArr.map((ele, idx) => {
            ...
            if (obj['指数简称'] == 'MicroLED概念') debugger
