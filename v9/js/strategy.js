...
function evaluateBlockStrongV9(item, dates) {
    ...
    // 1. 趋势确认：站上 21 日线 + 5 日线向上（过滤超跌反弹）
    const trendUp = item.M01 > item.M21 && item.M05 > item.prevM05

    // 2. 今日领涨：板块 09:35 涨幅全市场排名前 5
    const rank0935 = item['09:35涨跌幅排名'] || 9999
    const mainLine = rank0935 <= 5

    // 3. 近期强势：板块昨日上涨
    const pd1Chg = item[pd1]?.涨跌幅 || 0
    const recentStrong = pd1Chg > 0
...
function getQuestions(type, datas, BlockType, BlockName) {
    ...
    if (type === 'block-行业') {
        questions[0] = `${td}涨跌幅;${td} 09:35涨跌幅降序;${td} 09:35资金流向大单净额;${td} 09:33涨跌幅资金流向大单净额;${pd2}涨跌幅成交量大单净额;${td}前3交易日涨跌幅;${td}前3交易日资金流向;${pd1}前5交易日区间最高价;${pd1}资金流向大单净额;二级行业`
        ...
        questions[0] = `${td}涨跌幅;${td} 09:35涨跌幅降序;${td} 09:35资金流向大单净额;${td} 09:33涨跌幅资金流向大单净额;${pd2}涨跌幅成交量大单净额;${td}前3交易日涨跌幅;${td}前3交易日资金流向;${pd1}前5交易日区间最高价;${pd1}资金流向大单净额;概念`
