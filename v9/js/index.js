    ...
    const module = reactive({
        ...
        getData: async (dates, cacheSubKey, questionsBuilder, cache) => {
            ...
            for (let i = 0; i < questions.length; i++) {
                ...
                try {
                    results[i] = await fetchWithRetry(requestType, questions[i], {
                        maxRetries: MAX_RETRY_ATTEMPTS, retryDelay: 1000,
            ...
            if (successResults.length === questions.length) {
                cache[targetKey] = successResults
                // 缓存已禁用
...
const Stocks = createDataModule({
    ...
    dataHandler: async (results, dates) => {
        ...
        m0.forEach((v0, code) => {
            let merged = { ...v0.item }
            merged['09:35涨跌幅排名'] = v0.rank0935
            if (m1.has(code)) {
                Object.assign(merged, m1.get(code).item)
                merged['昨日涨跌幅排名'] = m1.get(code).rankPd1
            } else {
                merged['昨日涨跌幅排名'] = 9999
            }
 ...
 * App 初始化
 * ================================================================ */
/* V9 Block 强势判断包装：优先使用 V9 版主线判断，回退到 V7 版 */
function evalBlockStrongV9(item, dates) {
    if (typeof evaluateBlockStrongV9 === 'function') {
        return evaluateBlockStrongV9(item, dates).isStrong
    }
    return evaluateBlockStrong(item, dates).isStrong
}
    ...
    setup() {
        ...
        const strongIndustriesCount = computed(() => {
            return Industries.Data[0].filters.filter((item) =>
                evalBlockStrongV9(item, Dates.shareDate)
            ).length
        })
        ...
        const displayIndustries = computed(() => {
            ...
            if (MatchChart.industryFilterMode === 'strong') {
                result = result.filter((item) => evalBlockStrongV9(item, Dates.shareDate))
            }
        ...
        // --- 强势 Stock 数量 ---
        const v9Ctx = computed(() => {
            const row = Stocks.currentBlockRow
            if (!row) return null
            const { td } = Dates.shareDate
            return {
                blockStrong: evalBlockStrongV9(row, Dates.shareDate),
                blockChg0935: row[`${td} 09:35`]?.涨跌幅 || 0,
                blockRank0935: row['09:35涨跌幅排名'] || 9999,
                nStrongBlocks: (strongIndustriesCount.value || 0) + (strongConceptsCount.value || 0),
                marketOk: true,
            }
        })
        ...
        const strongStocksCount = computed(() => {
            ...
            const ctx = v9Ctx.value
            if (!ctx) return 0
            return Stocks.Data[0].filters.filter((stock) =>
                StrategyV9.evaluateStockV9(stock, Dates.shareDate, ctx).isHit
            ).length
        })
        ...
        const displayStocks = computed(() => {
            ...
            if (ctx) {
                result = result.map((s) => {
                    s.__v9 = StrategyV9.evaluateStockV9(s, Dates.shareDate, ctx)
                    return s
                })
                if (MatchChart.stockFilterMode === 'strong') {
                    result = result.filter((stock) => stock.__v9.isHit)
                }
                result.sort((a, b) => (b.__v9.score - a.__v9.score))
            }
        ...
        const handleIndustryRowClick = async (row) => {
            ...
            Stocks.currentBlockRow = row
            const blockName = row['指数简称']
            await Stocks.fetchByBlock(blockName, '行业', Dates.shareDate)

            if (DEBUG) {
                const result = (typeof evaluateBlockStrongV9 === 'function' ? evaluateBlockStrongV9(row, Dates.shareDate) : evaluateBlockStrong(row, Dates.shareDate))
                console.log(`=== Block评估 - ${blockName} ===`, {
                    ...result.conditions,
        ...
        const handleStockRowClick = (row) => {
            ...
            if (DEBUG) {
                const result = evaluateStockStrong(row, Dates.shareDate)

                console.log(`=== Stock评估 - ${stockName} ===`, {
