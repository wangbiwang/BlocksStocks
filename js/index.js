...
const Questions = reactive({
    block: [
        `当日涨跌幅资金流向大单净额大单净量收盘价;09:30涨跌幅；09:31涨跌幅资金流向大单净额;09:33涨跌幅资金流向大单净额;09:35${text01}；前1交易日涨跌幅资金流向大单净额大单净量；前5交易日区间最高价不复权；二级行业`,
        `当日涨跌幅资金流向大单净额大单净量收盘价;09:35${text01}；${text1}；${text2}；前1交易日收盘价；二级行业`,
        `当日涨跌幅资金流向大单净额大单净量收盘价;09:30涨跌幅；09:31涨跌幅资金流向大单净额;09:33涨跌幅资金流向大单净额;09:35${text01}；前1交易日涨跌幅资金流向大单净额大单净量；前5交易日区间最高价不复权；概念`,
        `当日涨跌幅资金流向大单净额大单净量收盘价;09:35${text01}；${text1}；${text2}；前1交易日收盘价；概念`,
    ],
    stock: [
        ...
        `前1交易日热度排名升序前5交易日区间最高价不复权；${text1}；${text2}；前1交易日收盘价前复权；行业概念`,
    ],
})
//指数板块相关
...
function handleBlocksData(res) {
    function handleArr(arr1, arr2) {
        ...
        return resArr.map((ele) => {
            ...
            obj['放量达成'] =
                (obj['放量'] > 0 && obj[pd1]['涨跌幅'] > 1 && obj[pd1]['大单净额'] > 0) ||
                (obj[pd1]['涨跌幅'] > 1 && obj[pd1]['资金流向'] > 0 && obj[pd1]['大单净额'] > obj[pd1]['资金流向'])
            //&& (obj[pd1]['大单净量'] ? obj[pd1]['大单净量'] > 0.1 : true)
            ...
            let 扣分 = [
                obj['09:35']['大单净额'] < 0 && obj['09:35']['资金流向'] < 0 ? -5 : 0,
                obj['09:31']['大单净额'] > 0 && obj['09:35']['大单净额'] < 0 ? -5 : 0,
...
function handleStocksData(res) {
    ...
    Stocks.Data[0].base = res[0]
        ...
        .map((ele) => {
            ...
            obj['放量达成'] = obj['放量'] > 0 && obj[pd1]['涨跌幅'] > 2 && obj[pd1]['大单净量'] > 0.1
            ...
            let 扣分 = [
                ...
                obj['09:31']['大单净额'] > 0 && obj['09:35']['大单净额'] < 0 ? -5 : 0,
                obj['09:35']['涨跌幅'] < 2 ? -5 : 0,
                Blocks.checked.item && Blocks.checked.item['09:35']['涨跌幅'] > obj['09:35']['涨跌幅'] ? -5 : 0,
            ].reduce((accumulator, currentValue) => accumulator + currentValue, 0)
            obj['9:35打分'] =
                // `${长期趋势} + ${短期趋势}  + ${扣分}=` +
                Number(长期趋势 + 短期趋势 + 扣分)
